...
- `src/render/environment/forestRoad.ts` — R4.5 Wave 2B map2-farm-only ground composition: depth tone strips + dirt-road S-curve, built once, gated by `forestRoadActiveForZone`; exposes `roadCenterXAt`/`roadHalfWidthAt` for 2C prop placement.
- `src/render/environment/mapProps.ts` — R4.5 Wave 2C map2-farm-only visual world props (trees/rocks/lamp/sign/gate fragment + near grass), deterministic hash placement, standing props join the shared `entities` depth-sort domain, gated by `mapPropsActiveForZone`.
- `src/render/environment/bossArena.ts` — boss-room-only fixed gate-pillar + lintel + vignette framing, built once.
- `src/render/environment/__tests__/` — pins grand-expansion biomes, town llama, town honor board, asura zones, gate hit-test, gate lock overlay, gate props, parallax layer, terrain ground, forest road, map props; 11 files.
