...
- [x] เทส 2123→**2163** (+40: gate 29, worldMapModel 14, relay counts, cache) · tsc/eslint/next build clean · patch-notes **2026-07-09m** (4 ข้อ th/en)
- [x] **MERGED TO MAIN (PR #44, เจ้าของยืนยันล่วงหน้า "ฝากเมิจ")** — deploy ยังค้าง: **relay ก่อน (counts endpoint) → web · ไม่มี db push** (บอทปิด default + ประตู จะเห็นบน prod หลัง deploy เท่านั้น)

## ✅ M8.9 — R2 "UI Sweep" (เสร็จ 2026-07-09 บน develop — รอเจ้าของเล่นจริง + confirm merge)

- [x] **แก้ตามเจ้าของหลัง R1**: **ประตู = เดินไปก่อนแล้วค่อยข้าม** (gateTrip state machine mirror smithTrip: moveTo → ถึงใน 40px → walkToZone ครั้งเดียว; ยกเลิกเมื่อสั่งอย่างอื่น/คุย NPC/ตาย/วาร์ป/หมดเวลา 20s; mutual-cancel กับ smithTrip) + **บอทปิด default ทุกครั้งที่เข้าเกม** (queueSetAutoHunt(false) หลัง offline replay — offline income ของช่วงที่ผ่านมาไม่กระทบ; FTUE copy สอน tap-to-move + เปิด AUTO เอง; **หมายเหตุพฤติกรรม**: จบเซสชันโดยบอทปิด → ช่วง offline ถัดไปฮีโร่ยืนเฉย เพราะ replay ใช้สถานะที่เซฟ)
- [x] **W1 primitives ชุด 2**: Tab/TabRow (ม่วง active) · CurrencyChip · StatBar · ItemTile (สีจาก labels.ts) · Toast · ConfirmPopup — กระเป๋า grid + ร้านป้าปุ๊ 3 แท็บใช้ชุดใหม่ (พฤติกรรมเดิม; useConfirmGuard two-tap คงไว้ — เปลี่ยนเป็น modal = เปลี่ยนพฤติกรรม)
- [x] **W2 HUD ถาวร**: portrait block (roundel คลาส + Lv + พลังต่อสู้ + HP/MP/EXP StatBar, ความสูงมือถือเท่าเดิม) · เงิน/หิน → CurrencyChip · ช่องสกิลเลข 1-N · **AUTO ย้ายมาแถวสกิล** ตอนปิด = gold invite pill · ยา/ยันต์ → quick-slot tiles
- [x] **W3 ตีบวก + อุปกรณ์**: RefinePanel โชว์ **โอกาสสำเร็จ %** (single-source engine/config/refine.ts — server อ่านตารางเดียวกันอยู่แล้ว ไม่มี drift) + ATK ก่อน→หลัง — reveal-on-final-strike ไม่แตะ · EquipmentDoll ใหม่ (roundel กลาง + ช่อง ItemTile ขนาบ + stat block 4 stat; ไม่มี AGI — engine มี 4 ไม่แต่งเพิ่ม; มือถือ = แถบเลื่อน แก้ overflow 375px เดิม)
- [x] **W4 สกิล/บอท/อันดับ**: SkillDetailModal (list+detail จาก skillStatParts — ไม่มีเลเวลสกิล ตาม backlog) · BotSettingsModal 3 หมวดตาม mockup + SkillAutoSlotPicker แตะติ๊ก (ทางเข้า ⚙ ข้าง AUTO ที่เดียว) · HOF เปลือกทอง + TabRow (โพเดียม logic PRs #29-30 ไม่แตะ)
- [x] **W5 จอใหญ่ desktop**: เปลือก HUD max-w-3xl → 5xl/6xl/7xl · GoalLadder **portal** ซ้อนบนสนาม (md+; portal ไม่ใช่ show/hide — กัน FTUE anchor จับตัวซ่อน) · มือถือ DOM เดิม · จอสูง ~16:9 จริง = รอ R4 x,y (ต้องต่อเพดานฟ้าฝั่ง render)
- [x] เทส **2188/2188** (+25 gateTrip/boot-bot-off) · tsc/eslint/next build clean · patch-notes **2026-07-09n** (5 ข้อ th/en)
- [ ] **ค้าง**: เจ้าของเล่นจริง — เดินไปประตูแล้วค่อยข้าม feel · แผงใหม่ทุกบานบนมือถือ (doll strip / RefinePanel / Bot panel) · ตำแหน่ง AUTO ใหม่ — merge เข้า main เมื่อเจ้าของยืนยัน (หมายเหตุ: เรื่อง "บอทไม่ปิด default" ที่รายงานรอบนี้ = บั๊กท่อ input ระดับราก แก้ใน M8.10 W0)

## ✅ M8.10 — R2.5 "Game Screen ตาม ref" + แก้ท่อ input รั่ว (เสร็จ 2026-07-09 บน develop, plan whimsical-frolicking-naur v3 — รอเจ้าของเล่นจริง)

> **ที่มา:** เจ้าของเล่น R2 แล้ว "ไม่เห็นตรง ref เลย" — วินิจฉัย: R2 ทาสีแผงแต่โครงหน้าจอเล่นเกมยังเดิม (แถบ 3:1 + UI รอบนอก) ขณะที่หัวใจของ ref = **เกมเต็มจอ + HUD ลอยทับทั้งหมด** · เจ้าของเคาะ: เต็มจอทั้งมือถือ+desktop · เมนูครบชุด + ปุ่ม NPC = สั่งเดินไปหา (เคาะใหม่ทับกติกา "ห้ามปุ่มร้านค้าบน HUD" — อนุญาตเพราะปุ่มแค่เดินไป ไม่เปิดแผงทางไกล, บันทึกใน ui-reference-map.md) · minimap-lite ตอนนี้ · ตัวละคร = code-drawn เดิม

- [x] **W0 แก้ท่อ input รั่ว (บั๊กจริงจาก localhost เจ้าของ)**: solo frame path ดูดคิว pendingInput ก่อนเช็ค step count — เฟรม 0-step (จอ 90Hz+ เจอประจำ, เฟรมแรกหลัง boot แทบการันตี) = intent หายถาวร → คำสั่งปิดบอทตอน boot หาย + tap หายประปราย · fix = pure `drainSoloFrame` (นับ steps ก่อน, drain เฉพาะ steps>0, ไม่งั้นคิวคงอยู่) · cohort path audit แล้ว = zero-loss โดยดีไซน์ (drain ที่ issue boundary เท่านั้น) ไม่แตะ · เทส exactly-once ข้ามเฟรม 120Hz
- [x] **W1 render เต็มจอ**: `computeFullscreenTransform` (BAND_BIAS 0.58 / SKY_BLEED / GROUND_BLEED / BLEED_X, ที่ 3:1 = เท่า transform เดิมเป๊ะ) · filterArea+camera mask ขยายตาม visible rect (mask ยัง camera-ON only = OFF-identity) · camera.ts รับ viewW (posX ตั้งใจคง WORLD_WIDTH/2 — พิสูจน์แล้วว่า base transform จัดกลางจอให้เอง) · sky 7→16 bands + groundBand depth shading + bossArena ต่อกรอบถึง bleed (ห้องบอสไม่โชว์ฟ้าดิบ) · `setFilterOrigin` กัน shockwave clip บริเวณ bleed · +33 เทส fullscreenLayout
- [x] **W2 overlay HUD ทั้งสองแพลตฟอร์ม**: page h-dvh + canvas absolute inset-0, **ลบ layout กรอบเดิม** (HudBar.tsx + WalkControls.tsx ตาย — เนื้อย้ายบ้าน) · HeroPortraitCard ซ้ายบน (สกัดจาก SkillBar) · คอลัมน์ขวาบน CurrencyChipsRow + แถวไอคอน IconTileButton (กระเป๋า/HOF/เพื่อน/codex/ตั้งค่า/asura/แผนที่/วาร์ป/**ตัวละคร** — CharacterPanel ใหม่กลืน StatPanel/EquippedLoadout/SwitchCharacter) · GoalLadder portal ทุก viewport (มือถือ compact แตะกาง, FTUE บังคับกาง) · skill dock กลางล่าง + ExpClockStrip ขอบล่าง · FTUE re-anchor (stat-panel→character-menu, settings-row→menu-row th/en) · z-ladder 0..75 documented · RTL smoke + jsdom devDeps
- [x] **W3 npcTrip + ปุ่ม NPC + minimap-lite**: `npcTrip.ts` แทน smithTrip (pahpu/lungdueng/elder, เปิดแผงผ่าน seam เดียวกับ tap-to-talk, ปิดช่องโหว่ cancel ตอน fastTravel/warp ที่ smithTrip ไม่เคยมี) · `NpcTripButtons` 3 ปุ่ม walk-order-only (dim+toast ตอนบอส/เดินทาง — ไม่ใช้ native disabled ตามกติกาเดิม) · `MiniMapCard` มุมขวาบน (ชื่อโซน + SVG strip จุดตัวเอง/ประตู+ล็อค/NPC + 👥 นับสดผ่าน useZoneCounts {open,pollMs} 30s หยุดตอน tab ซ่อน) แตะ = เปิดแผนที่โลก · RefineButton ค้างไว้ unused รอเจ้าของยืนยันถอน
- [x] **กติกาใหม่ Docs discipline ใช้แล้วทั้งรอบ**: docs/CODEMAP.md sync ทุก add/delete/move (เทส codemap เขียว)
...
- [x] **Wave B — render cutover (#51)**: seam `worldFxContext.depthOf` อ่าน engine `planeY` เมื่อ flag `worldDepthFromEngineY` (render-side, default ON) — Hero/Enemy อ่านจาก entity, ghost ผ่าน `scatterPlaneY(cid)` (town-NPC ไม่ใช้ — static); `planeToDepth` = inverse **bit-exact** (band width 64 = 2^n) → footY/scale/zIndex เท่าเดิมเป๊ะ, hash เดิมเป็น fallback + OFF path ยังเลือกได้. `buildCohortState` (app) fan party ด้วย `heroPlaneY(cls, cohortIndex, size)` (solo rebuild = solo row เดิม). Stage boss/world boss/town NPC = static `DEPTH_NEUTRAL` เดิม (ไม่ scatter, planeY ไม่ถูกอ่าน — วาดหน้าสุดด้วย fixed zIndex) → identity trivial. Identity test (temporary, retire Wave C) พิสูจน์ ON===OFF ทุก class: hero solo+party 2–6, enemy, bossAdds, ghost, boss/NPC. Combat/movement/`Entity.y`/SAVE_VERSION ไม่แตะ · sim gates ไม่ขยับ
- [x] **Wave C0 — retire the Wave-B cutover scaffold (#51)**: ลบ flag `worldDepthFromEngineY` + OFF branch + render-side depth-ASSIGNMENT (`depthAssign` heroDepth/enemyDepth/ghostDepth + HERO_* consts) — engine `planeY` เป็น path เดียวที่ seam. `depthAssign` เหลือ `hashUnit` (terrain-preset/weather-window + seam no-`planeY` fallback `hashUnit(id)`). `FxController` enemy/hero lift ส่ง `planeY` ตรง (entity อยู่ในมือ). ลบ identity test; plane.test เลิก pin ค่าคงที่ render ที่ตายแล้ว (band ยัง pin กับ depthBand ที่ยังอยู่). **Behaviour-neutral เป๊ะ** (flag default ON อยู่แล้ว → ลบ OFF path ไม่เปลี่ยนภาพ) · combat/movement/`Entity.y`/SAVE_VERSION/sim gates ไม่แตะ. Stage/world boss/town NPC ยัง static `DEPTH_NEUTRAL` ตลอด R4 (owner-confirmed; revisit R5 boss-telegraph)
- [x] **Wave C1 — hero y steering (#51)**: hero `planeY` กลายเป็น MUTABLE ต่อ step (เฉพาะ hero — enemy/boss/worldBoss ยัง static scatter, owner-confirmed) ผ่าน pure helper `systems/plane.stepPlaneY(current,target,dt)` (delta clamp + `CONFIG.plane.yArriveEps` snap-at-arrival, ease ที่ `CONFIG.plane.ySpeed` — ไม่ clamp ค่า y กับ band ตรงๆ: target มาจาก `heroPlaneY`/`enemy.planeY` ที่อยู่ใน band อยู่แล้ว). Wiring อยู่ท้าย per-hero loop ใน `combat.updateHeroes` — **unconditional cosmetic steering** ที่รันไม่ว่าจะเข้า x move/attack branch ไหน, ไม่ได้เพิ่ม/ลบ/สลับ x-move หรือ attack call ใดๆ. เป้า y = แถวของ **มอบ farm ที่ engage** (`aimTarget` ที่เป็น `state.enemies` member) มิฉะนั้น home row = `heroPlaneY(cls, cohortIndex, heroes.length)` (recompute stateless). **BOSS/WORLD-BOSS phase → home row เสมอ** (บอสวาดบน static DEPTH_NEUTRAL ไม่อ่าน stamped near-row; ถ้า steer ไป boss.planeY จะไปเลนที่บอสไม่ได้วาด). **IRON invariant**: y ไม่ gate combat เลย — targeting/range/cooldown/skills ไม่อ่าน `planeY` (targeting.ts/skills.ts ไม่แตะ). Kill-time fx: enemy ยังใช้ spawn-hash fallback (มอบไม่ขยับ → เป๊ะ), hero fx อ่าน live `planeY` ผ่าน `FxController.heroLift` (hero ไม่เคยออกจาก state) → **ไม่มี event payload เปลี่ยน**. `Entity.y`/SAVE_VERSION ไม่แตะ · **sim gates byte-identical** (พิสูจน์ diff before/after)
