...
- [ ] **Phase 0 — Account system (สเปกเจ้าของเคาะ 2026-07-08 — แบบง่ายที่สุด)**: เพิ่มหน้า **Settings → My Account** — ผู้เล่น cookie นิรนามกด **register ด้วย email + password ในนั้นเลย** (ผูกเข้า `User` แถวเดิม → save/ตัวละครไม่หายโดยธรรมชาติ) · **ไม่ต้อง verify email, ไม่มีกฎ password ยุ่งยาก — validation เดียวคือ email ห้ามซ้ำ** (unique constraint + เช็คฟอร์แมตพื้นฐาน; password เก็บแบบ hash เสมอ) · login ด้วย email+password จากเครื่องอื่น = สลับ cookie ไปชี้ account นั้น · ผู้เล่นที่ไม่สมัครยังเล่นต่อได้ตามเดิม (ฟีเจอร์เพื่อนถึงค่อยบังคับมี account) · identity ทั้งหมดอยู่หลัง `src/server/identity.ts` (จุด swap ที่เตรียมไว้แล้ว) · friend code ต่อ account + display name
