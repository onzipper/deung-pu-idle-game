...
/** Build a hero at its formation home position, at full (upgraded) HP. */
export function makeHero(id: number, cls: HeroClass, up: Upgrades): Hero {
  const t = HERO_TYPES[cls];
  const maxHp = heroMaxHp(up);
  return {
    id,
    cls,
