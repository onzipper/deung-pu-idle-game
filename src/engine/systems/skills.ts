...
export function castSkill(state: GameState, hero: Hero): boolean {
  ...
  if (hero.cls === "swordsman") {
    const dmg = Math.round(heroAtk(hero.cls, state.upgrades, hero.level, hero.tier) * sk.mult);
    ...
    state.projectiles.push({
      ...
      damage: Math.round(heroAtk(hero.cls, state.upgrades, hero.level, hero.tier) * sk.mult),
      speed: sk.projSpeed,
      targetId: null,
      tx,
