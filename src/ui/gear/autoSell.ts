/**
 * M7.5→M7.9 auto-dispose rules — pure selection of which owned instances a
 * town-trip (manual bulk path OR the bot's `townArrived` reason "sell"/
 * "restockSell" executor in `GameClient.tsx`) should SELL. No React/fetch here
 * (headlessly testable, `__tests__/autoSell.test.ts`).
 *
 * Owner request 2026-07-08 (หินเสริมพลัง final wave): salvage is RETIRED —
 * refine stones now drop directly from mobs (`engine/systems/gear.ts`'s
 * `stoneDrop`), so the bot's per-rarity action is a plain 2-way
 * "off" | "sell" toggle now (was a 3-way "off"|"sell"|"salvage" through
 * M7.7-M7.9). `AutoSellRules.epic` keeps its own "กันของดี" keep-guard
 * FORCED ON regardless of the shared `keepBetterStat` toggle — an epic may
 * only be sold when it's strictly worse than what's equipped in its slot AND
 * isn't the best class-equippable candidate for an empty slot.
 *
 * `keepBetterStat` (plus the forced epic case) guards whether an instance is
 * a disposal CANDIDATE at all — it protects:
...
export interface AutoSellRules {
  ...
  rare: AutoSellAction;
  /** Epic's own toggle (M7.9 "option A"). Optional + defaults "off" so any
   * caller (existing tests, stale localStorage) that omits this field keeps
   * the old "epic never disposes" behavior byte-identical. */
  epic?: AutoSellAction;
 ...
 * braces — the server's sell endpoint rejects those anyway,
 * `reason: "equipped"`). A template missing from `templates` (stale/retired)
 * is skipped defensively rather than crashing the sweep. `heroClass` scopes
 * the empty-slot best-backup pick to items the hero can actually wear.
 */
export function selectAutoSellIds(
  ...
  for (const item of items) {
    ...
    const t = templates[item.templateId];
    if (!t) continue;
    if (actionFor(t.rarity, rules) === "off") continue;
