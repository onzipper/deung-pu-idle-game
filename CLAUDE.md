...
Status (2026-07-06): **M1–M5 complete** (M5 = single-character pivot: char creation + 3 slots live on the DB, base stats, mana + 3-skill kits, class-change quest, FTUE rework; SAVE at v9). **M6 nearly done** — zone/world engine, hunting fields (15-18 mobs), town + NPC potions/auto-use, per-map theming + boss arenas, HUD readability pass all landed; remaining: goal-ladder UI + two hunt follow-ups (archer AoE self-aggro walls at s13 — needs gradual re-entry fill / AoE-aggro rule; min-spacing spawn placement). **M6.5 art exploration CLOSED** — owner keeps the original procedural style; /proto deleted; gear-wow visuals (sparkling armor, Super-Saiyan weapon aura) build on the existing rigs in M7. Next: **M7 Gear & Drops ⭐** (ItemInstance/ItemEvent tables already applied — `docs/persistence-m7.md`) → M8 Party (needs VPS/websocket decision first) → M9 Economy & Competition. PvP cut; prestige on hold. Balance baseline = `docs/balance-m6.md`.
