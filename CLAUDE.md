...
Status (2026-07-06): **M1–M7.5 complete** (M5 = single-character pivot · M6 = zone/world + hunting fields + town + goal-ladder HUD · M7 = gear & drops end-to-end: 27-template catalog, deterministic stateless drops, server-authoritative ItemInstance/ItemEvent ledger with idempotent claims, inventory/equip UI, paper-doll on the original rigs · M7.5 = NPC sell [town-only, atomic no-double-credit] + auto-sell rules + town-trip bots [restock/sell, scroll-warp, SAVE v11] + fast travel [free to unlocked zones, 1.75s channel] + biome gate archways/grand boss door/warp-portal fx + RO-grid inventory [stack ×N, stat-delta compare, cap 100] + HUD auto-hunt toggle [SAVE v12]). **M6.5 art exploration CLOSED** — owner keeps the original procedural style. In flight: **M7.7 Skill Spectacle & World Heat** — owner-locked scope in ROADMAP (skills เบิ้ม + tier-2 field-wide ultimates, mana = pacing governor, denser fields 17/19/21 + killGoal pacing, survivor-retaliation aggro rule, boss buffs DEFERRED); class-bug fix + repair landed (1d5ba4d); NEXT SESSION: re-launch the engine wave (the full agent brief = the M7.7 ROADMAP block + sim gates: class change ~s5, s15 wall intact, mana potions become a real sink), then render spectacle wave, then a small mana-readout UI pass. After that: **M7.6 Refine ตีบวก** (owner-locked in ROADMAP; needs additive DB column refineLevel + SAVE v13 + full sim sweep; start fresh-quota) → M8 Party (FIRST: VPS/websocket infra spike decision) → M9 Economy & Competition. PvP cut; prestige on hold. Balance baselines = `docs/balance-m6.md` (world) + `docs/balance-m7.md` (gear/vendor).
